...
import type { DataFieldType } from '~/types/data-context'
 ...
 * 数据源分组配置
 */
export const DATA_SOURCE_GROUP_CONFIG: Record<BindableFieldSource, { label: string; icon: string; order: number }> = {
...
export interface CategoryGroup {
  key: string           // 分类键
  label: string         // 分类显示名称
  fields: BindableFieldExt[]  // 该分类下的字段
}
 ...
 * 将字段列表按数据源和分类进行层级分组
 */
export function groupFieldsHierarchically(fields: BindableFieldExt[]): SourceGroup[] {
  const sourceMap = new Map<BindableFieldSource, Map<string, BindableFieldExt[]>>()
  
  for (const field of fields) {
    if (!sourceMap.has(field.source)) {
      sourceMap.set(field.source, new Map())
    }
    const categoryMap = sourceMap.get(field.source)!
    const categoryKey = field.category || 'other'
    if (!categoryMap.has(categoryKey)) {
      categoryMap.set(categoryKey, [])
    }
    categoryMap.get(categoryKey)!.push(field)
  }
  ...
  for (const source of sortedSources) {
    const categoryMap = sourceMap.get(source)!
    const config = DATA_SOURCE_GROUP_CONFIG[source]
    
    const categories: CategoryGroup[] = []
    for (const [categoryKey, categoryFields] of categoryMap) {
      const firstField = categoryFields[0]
      categories.push({
        key: categoryKey,
        label: firstField?.categoryLabel || categoryKey,
