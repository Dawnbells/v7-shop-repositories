 ...
 * 在请求到达页面前进行风控检查，将结果注入到 event.context
 */
 ...
 */
function getOrCreateFingerprint(event: any): string {
  // 从 Cookie 获取
  ...
  console.log("[Cloak Middleware] Generated new fingerprint:", newFp);
  return newFp;
}
 ...
 * 构建斗篷检查请求
 */
function buildCloakRequest(event: any): CloakCheckRequest {
...
async function performCloakCheck(
  ...
  try {
    ...
    console.log(`[Cloak Middleware] Remote check completed in ${elapsed}ms`);
    ...
    const elapsed = Date.now() - startTime;
    console.error(`[Cloak Middleware] Check failed after ${elapsed}ms:`, error);
...
export default defineEventHandler(async (event) => {
  ...
  console.log("[Cloak Middleware] path:", path);

  // 获取降级策略
  const pageContext = getPageContext(event);
  const fallbackPage = pageContext.company?.cloakFallback ?? CloakPage.LAND;
  ...
  if (previewPage) {
    ...
    console.log("[Cloak Middleware] request:", request);
    cloakResult = await performCloakCheck(request, fallbackPage);
    console.log("[Cloak Middleware] result:", cloakResult);
    ...
    console.log("[Cloak Middleware] Saved cloak result to cookie");
  } else {
    // 非产品路由：从 cookie 读取
