<template>
  <div class="article-container auto-height-container">
    <vab-query-form>
    ...
    >
      ...
      <el-table-column align="center" label="文章名字" prop="name" />
      ...
      <el-table-column align="center" label="语言">
        <template #default="{ row }">
          <el-space wrap>
            <el-tooltip
              class="box-item"
              :content="row.language.name + '(' + row.language.code + ')'"
              effect="dark"
              placement="top"
            >
              <el-tag type="primary">{{ row.language.cname }}</el-tag>
            </el-tooltip>
          </el-space>
        </template>
      </el-table-column>
      <el-table-column align="center" label="文章类型" prop="articleType">
        <template #default="{ row }">
          <el-tag v-if="row.articleType === 'PROTOCOL'" type="primary">协议</el-tag>
          <el-tag v-else type="primary">常规</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" label="文章说明" prop="description" />
    ...
    />
    <article-edit ref="editRef" @fetch-data="fetchData" />
  </div>
</template>

<script lang="ts" setup>
import { Delete, Plus, Search } from '@element-plus/icons-vue'
import { doDelete, page, switchValidity } from '/@/api/article'
