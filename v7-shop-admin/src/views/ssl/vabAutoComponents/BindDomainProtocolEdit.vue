        ...
        <el-row :gutter="20">
          <el-col :md="12" :sm="12" :xs="24">
            <el-form-item label="域名" prop="name">
              <el-input v-model.trim="form.name" disabled placeholder="顶级域名" />
            </el-form-item>
          </el-col>
          <el-col :md="12" :sm="12" :xs="24">
...
import { bindProtocol } from '/@/api/topLevelDomain'
...
const rules = reactive<any>({
  ...
  protocolId: [{ required: true, trigger: 'change', message: '请选择绑定协议' }],
})
  ...
  formRef.value.validate(async (valid: any) => {
    if (valid) {
      try {
        ...
        const { msg }: any = await bindProtocol(form)
...
.bind-protocol-form {
  ...
  .placeholder-card {
    ...
    .el-col {
      margin-bottom: 24px; // 进一步加大间距
    }

    :deep(.el-form-item) {
      margin-bottom: 0; // 由 el-col 的 margin-bottom 控制
    }
