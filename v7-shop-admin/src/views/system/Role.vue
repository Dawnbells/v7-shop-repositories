<template>
  <div class="role-container auto-height-container">
    ...
    <el-table ref="tableRef" v-loading="listLoading" border :data="list" @selection-change="setSelectRows">
      ...
      <el-table-column align="center" label="类型" prop="systemUserType">
        <template #default="{ row }">
          <el-tag>{{ getUserTypeLabel(row.systemUserType) }}</el-tag>
        </template>
      </el-table-column>
...
const userTypeOptions = [
  { label: '管理员', value: 'COMPANY_ADMIN' },
  { label: '部门超级管理员', value: 'DEEP_DEPARTMENT_MANAGER' },
  { label: '部门管理员', value: 'DEPARTMENT_MANAGER' },
  { label: '员工', value: 'EMPLOYEE' },
]
// Add this computed property
const getUserTypeLabel = (type: string) => {
  return userTypeOptions.find((option) => option.value === type)?.label || type
}
